...
1. Column filters from the downloaded data to final dataset
...
Now, per step 2, only the measurements on the mean and standard deviation were extracted. So the script extracts only the columns conatining `Subject|Activity|std|mean`. 
The function call used in the script is case sensitive and excludes the occurrenes of upper case `Mean`, which is used in the `angle` columns which I do not want anyways because the mean is a parameter.
